...
export async function crearSolicitud(datos) {
  ...
  // Si lo captura quien valida (Mantenimiento), nace aprobada: pasarla por su
  // propia bandeja sería validar dos veces lo mismo.
  if (datos.validada_por) {
    Object.assign(fila, {
      estado: 'aprobada',
      causa_validada_id: causa.id,
      validada_en: new Date().toISOString(),
      resuelta_por: String(datos.validada_por),
    });
  }
 ...
 */
export async function descartarSolicitud(folio, { por = '' } = {}) {
  const solicitud = await resolverSolicitud(folio, 'rechazada', { por });
  const { data: vigentes, error } = await supabase.from('planta_solicitudes')
    .select('folio').eq('activo_id', solicitud.activo_id).eq('cerrada', false)
    .neq('estado', 'rechazada').neq('folio', folio);
  if (error) fallar(error, 'No fue posible revisar otros reportes del activo');
  if (vigentes?.length) return { solicitud, maquina_liberada: false };

  const { data: estado, error: errEstado } = await supabase.from('planta_estados')
    .select('estado').eq('activo_id', solicitud.activo_id).maybeSingle();
  if (errEstado) fallar(errEstado, 'No fue posible leer el estado del activo');
  if (estado?.estado === 'STOP') await cambiarEstado(solicitud.activo_id, 'RUN');
  await cerrarSolicitudesDe(solicitud.activo_id);
  return { solicitud, maquina_liberada: true };
}
 ...
 * el tablero.
 */
export async function resolverPendiente(folio, resolucion, { por = '' } = {}) {
  const { data: actual, error } = await supabase
    .from('planta_solicitudes').select('estado').eq('folio', folio).maybeSingle();
  ...
  if (resolucion === 'rechazada') return descartarSolicitud(folio, { por });
  return { solicitud: await resolverSolicitud(folio, 'aprobada', { por }) };
}
