...
export async function generarAnalisis({ desde = null, hasta = null, enfoque = 'finanzas', proveedorForzado = null } = {}) {
  ...
  const proveedor = proveedorForzado || (esReportePdf
    ? (process.env.AI_REPORTE_PROVIDER || 'gemini')
    : await proveedorPara(enfoque));
...
export function crearPdf({ resumen, analisis }) {
  return new Promise((resolve, reject) => {
    ...
    const encabezado = () => {
      ...
      pdf.fillColor('#AEB8C5').font('Helvetica').fontSize(8.5).text(`EMITIDO ${emitido}`, W - X - 160, 53, { width: 160, align: 'right' });
...
async function reporteCacheadoReciente(desde, hasta) {
  let consulta = supabase.from('planta_reportes')
    .select('*, planta_analisis_ia(resultado)')
  ...
  return { ...reporte, analisis: planta_analisis_ia?.resultado ?? null };
}
...
export function destinatariosPredeterminadosWhatsApp(tipo = 'operaciones') {
  const prefijo = tipo === 'finanzas' ? 'WHATSAPP_FINANZAS_DESTINATARIO' : 'WHATSAPP_OPERACIONES_DESTINATARIO';
  const numerados = Array.from({ length: 4 }, (_, indice) => String(process.env[`${prefijo}${indice + 1}`] || '').trim())
    .filter(Boolean);
  if (numerados.length) return numerados;
  const individual = String(process.env[prefijo] || '').trim();
  if (individual) return [individual];
  return configuracion('WHATSAPP_ALERTAS_DESTINATARIOS').split(',').map((valor) => valor.trim()).filter(Boolean);
}

export function destinatarioPredeterminadoWhatsApp(tipo = 'operaciones') {
  return destinatariosPredeterminadosWhatsApp(tipo)[0];
}
...
function detalleErrorMeta(error) {
  ...
  const detalle = error.error_data?.details || error.error_data?.detail;
  return [error.message || 'Error de Meta', detalle, codigo && `código ${codigo}`, error.type].filter(Boolean).join(' · ');
}
...
function textoPlantilla(valor, limite = 1000) {
  // Meta valida la estructura final de la plantilla. Un valor con saltos de
  // línea cambia esa estructura y puede provocar el error 132018, aunque el
  // número de variables sea correcto. Las plantillas siguen mostrando el
  // contenido completo, pero cada parámetro viaja como una sola línea.
...
export function crearPlantillaMeta(nombreVariable, nombrePredeterminado, parametros = [], opciones = {}) {
  ...
  const nombre = process.env[nombreVariable] || nombrePredeterminado;
  const idiomaVariable = `${nombreVariable}_LANGUAGE`;
  const idiomaPredeterminado = nombre === 'downtimeos_alerta_paros' ? 'es_MX' : 'en_US';
  const idioma = opciones.idioma
    || process.env[idiomaVariable]
    || (process.env[nombreVariable] && process.env.META_WHATSAPP_TEMPLATE_LANGUAGE)
...
export async function enviarPorMeta({ destino, contenido, mediaUrl = null, interactivo = null, plantilla = null }) {
  ...
  const enviar = async (idioma) => {
    const cuerpo = { messaging_product: 'whatsapp', to: destino };
    if (plantilla) {
      cuerpo.type = 'template';
      cuerpo.template = { name: plantilla.nombre, language: { code: idioma || plantilla.idioma }, components: plantilla.componentes };
    } else if (interactivo) {
    ...
    if (!respuesta.ok) {
      const error = new Error(detalleErrorMeta(json?.error) || `HTTP ${respuesta.status}`);
      error.metaCode = Number(json?.error?.code);
      throw error;
    }
    return json;
  };
  ...
  let json;
  try {
    json = await enviar(plantilla?.idioma);
  } catch (error) {
    // Meta usa códigos locales distintos según la cuenta (por ejemplo en_US
    // o en). Si sólo falla la traducción, prueba el alias equivalente sin
    // tocar la plantilla ni sus parámetros. Cualquier otro error se conserva.
    const idioma = plantilla?.idioma;
    const alterno = idioma === 'en_US' ? 'en' : idioma === 'en' ? 'en_US' : idioma === 'es_MX' ? 'es' : idioma === 'es' ? 'es_MX' : null;
    if (!plantilla || error.metaCode !== 132001 || !alterno || alterno === idioma) throw error;
    json = await enviar(alterno);
  }
...
export async function enviarWhatsApp({ destinatario, contenido, reporteId = null, eventoFolio = null, interactivo = null, plantilla = null, destinatarioTipo = null }) {
  ...
  const destinos = (destinatario ? [destinatario] : destinatariosPredeterminadosWhatsApp(tipo)).map(telefonoWhatsApp);
  let mediaUrl = null;
  if (reporteId) {
  ...
  if (proveedor === 'meta' && reporteId && !plantilla && usaPlantillasMeta()) {
    // La plantilla aprobada de reporte tiene encabezado de texto ("Documento"),
    // no encabezado multimedia. Enviar un parámetro document provoca 132012.
    // El PDF viaja como enlace firmado dentro de la variable de texto.
    const contenidoConReporte = mediaUrl
      ? `${contenido}\n\nDescarga el PDF del reporte: ${mediaUrl}`
      : contenido;
    plantilla = crearPlantillaMeta('META_WHATSAPP_TEMPLATE_REPORTE', 'downtimeos_reporte_ejecutivo', [contenidoConReporte]);
  }
  ...
  return { ...envios[0], envios, destinatarios: destinos };
}
