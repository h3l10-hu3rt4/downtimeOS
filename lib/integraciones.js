...
export function crearPdf({ resumen, analisis }) {
  return new Promise((resolve, reject) => {
    ...
    const tituloSeccion = (titulo, y, etiqueta = '') => {
      ...
      // La caja de la etiqueta termina en el margen derecho (W - X), igual que la
      // línea divisoria. Con `W - X - 270` terminaba en el borde físico de la
      // hoja y el texto alineado a la derecha quedaba cortado.
      if (etiqueta) pdf.fillColor(C.gris).font('Courier').fontSize(7).text(etiqueta, X + 270, y + 3, { width: W - X * 2 - 270, align: 'right' });
