...
function integracionesExternasDesactivadas() {
  return String(process.env.INTEGRACIONES_EXTERNAS_DESACTIVADAS || '').toLowerCase() === 'true';
}

function exigirIntegracionesExternasActivas(servicio) {
  if (integracionesExternasDesactivadas()) {
    throw errorPeticion(`Las integraciones externas están desactivadas; no se puede usar ${servicio}.`, 503);
  }
}
...
export async function generarAnalisis({ desde = null, hasta = null, enfoque = 'finanzas' } = {}) {
  exigirIntegracionesExternasActivas('IA');
...
export async function crearReporte({ desde = null, hasta = null } = {}) {
  exigirIntegracionesExternasActivas('reportes PDF');
...
export async function enviarWhatsApp({ destinatario, contenido, reporteId = null, eventoFolio = null, interactivo = null }) {
  exigirIntegracionesExternasActivas('WhatsApp');
...
export async function alertaDeParos({ destinatario = null } = {}) {
  ...
  const mensaje = await enviarWhatsApp({ destinatario, contenido });
