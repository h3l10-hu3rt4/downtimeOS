...
/**
 * Guarda de servidor para los handlers administrativos. Complementa a
 * middleware.js: si el middleware cambia o no se ejecuta (por ejemplo en el
 * servidor local), el handler sigue sin entregar datos. Lanza un error con
 * `status` que `ruta()` convierte en JSON: 503 sin credenciales configuradas,
 * 401 sin sesión válida.
 */
export function exigirSesionAdministrador(req) {
  if (!administradorConfigurado()) {
    throw Object.assign(new Error('La administración no está configurada.'), { status: 503 });
  }
  if (!sesionAdministradorValida(req?.headers?.cookie ?? '')) {
    throw Object.assign(new Error('Sesión de administración requerida.'), { status: 401 });
  }
}
